...
| Optional | Why |
|----------|-----|
| `~/.research_db_keys.json` | Higher API rate limits + OMIM access (academic key). ClinPGx (ex-PharmGKB) is keyless. |
...
# Note: ClinPGx (successor to PharmGKB) is public/keyless — no pharmgkb key.
EOF
...
- FADS1 rs174547 C/T → example ALA → EPA/DHA conversion note
```
