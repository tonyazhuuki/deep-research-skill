---
name: deep-research
description: "Multi-agent iterative research pipeline using Eric Jang methodology. 10-18 specialized agents across 3 cycles produce consensus references, interaction maps, personalized syntheses, and action plans with domain-specific stress-testing, iterative deepening, and anti-pattern guards. v4.3 adds domain-specific study cards, live biomedical database lookups, and genome adapter."
version: "4.6"
license: MIT
---
...
| **DOMAIN_REVIEWER** | 1 | Domain-specific review: MEDICAL (health), MACRO (markets), MARKET (company), METHODOLOGY (science), CMO (marketing) |
| **FRONTIER_ANALYST** | 1 | Leading-indicator layer: tiers directions T0-T4 by evidence×promise, tracks resolving events for promising-but-unproven — recovers the frontier standard research discards |
...
- **Marketing:** "This tactic is documented publicly — what happens to its yield when everyone reading the same case study runs it?"
...
> Additive. **In plain terms:** `/research` now has a **marketing brain** — and it knows that marketing tactics expire. A new `marketing` domain adapter joins health/macro/company/science/creative.

### Marketing domain adapter (`domains/marketing.md`)
Marketing "truth" is often "what converts NOW", not a stable fact — so this adapter adds three things no other domain has:
- **Claim-layer freshness model:** every tactic card is DATED (`date_observed`) and LAYERED (L1 principle → L4 arbitrage) with explicit half-lives. The METHODOLOGIST auto-downgrades stale evidence (short-form/IG tactics decay fastest: −1 at 6 mo, cap C at 12 mo). Consensus outputs carry a `re_verify_by:` trigger; UPDATE mode / TEMPORAL DIFF is the refresh mechanism.
- **SCOUT-B (Buzz):** one SCOUT runs `/last30days` as a first-class evidence engine (mirror of health's SCOUT-D), routing through a curated per-quadrant source registry (`domains/marketing_sources.yaml`) — anchored to named subreddits/handles, never bare nouns.
- **Strategy/Performance × B2B/B2C quadrant model:** a first-class dimension (card fields `segment` + `marketing_mode`) that reshapes routing, sources, frameworks, and evidence expectations — with a **quadrant-transfer guard** (a B2C performance tactic may not be cited for a B2B brand-strategy question).

Plus a **Framework Library** (Ehrenberg-Bass / Binet & Field / Bass diffusion as graded L1 anchors; Bullseye / AARRR / JTBD / Dunford / Category Design as coverage-scaffolds or lenses — with hype-flags so lore can't launder into proof), a **CMO_REVIEWER** ("would this defend to a skeptical CMO"), an evidence ladder (measured lift > case study > practitioner testimony > vendor claim > anecdote) with a vendor-bias filter + correlation-vs-causation guard, and a Phase-2 **brand-context adapter** (Context Profile → `_brand_context.md`, mirror of the genome adapter). Wires into the Content Machine RESEARCH stage + AI Champion GTM deliverables. Full design: `90_meta/improvements/research_marketing_adapter_plan.md`.

## v4.5 Features

> Additive. **In plain terms:** the pipeline used to report only what's already *proven* — but Grade-A proof arrives 5-15 years late, so we were always the "catching-up tail," never the cutting edge. v4.5 adds a **Frontier Radar** that also flags directions that are *promising-but-not-yet-proven* — with the discipline to not become hype.

### ACTION MAPPER follow-up routing (queue hygiene, 2026-07-05)
The final ACTION MAPPER now **classifies every follow-up before filing it**, so operational tasks stop polluting the research queue. Three-way test: a genuine multi-agent research question → `research_queue.md`; a blood draw / panel / re-test → `lab_schedule.yaml`; a ≤15-min human action (call, email, booking, portal, purchase) → `ops_checklist.md`. Litmus: "does this need AI agents + hours of literature search?" — if no, it is lab or ops, not research. Previously the only sink for anything forward-looking was "new research idea → research_queue", so phone calls and lab orders piled up there.

### Frontier Radar (new mandatory layer)
New role **FRONTIER_ANALYST** (runs parallel with DEVIL'S ADVOCATE, every domain) + a mandatory `## Frontier Radar` section in every synthesis/consensus, plus `_frontier_radar.md`.
- **Two separate axes:** Axis 1 = evidence strength (the GRADE already assigned, never inflated) · Axis 2 = promise (mechanism + earliest human signal + serious adopters + trajectory + precedent).
- **Maturity tiers T0-T4:** T0 Established · T1 Emerging-Strong · T2 Promising-Early (the frontier quadrant: low evidence × high promise) · T3 Speculative-Watch · T4 Hype/Refuted.
- **Resolving event mandatory** for every T1-T3 item: the specific trial/readout/timeline that will move it or kill it (leading indicator). Absence of any registered trial after years = explicit anti-signal.
- **Anti-hype guardrails:** mechanism+registered-trial+real-sponsor ≠ mechanism+testimonial; base-rate calibration (Phase 1→approval ~10-20%); biomarker ≠ outcome (and flag when moving the biomarker is harmful); name the failed pivotal trials marketing ignores.
- First applied in `2026_07_peptides_landscape` (peptides field consensus).

## v4.4 Features

> All additive. **In plain terms:** sharper evidence grading + legal full-text retrieval.

### Two-axis evidence rubric ("Design First, but N Bounds the Claim")
METHODOLOGIST grades each finding on two separate questions, not one verdict:
- **Axis 1 — is it real / causal?** (design). Causation via randomization OR a converging Bradford-Hill case (very large RR ≥5–10× + dose-response + multi-design consistency + mechanism + temporality) that suspends the no-RCT penalty (smoking→lung-cancer = HIGH without an RCT). Small RR in a single cohort does not upgrade. Mechanism = hypothesis, not effect.
- **Axis 2 — how strong / applies to whom?** (sample size + representativeness). Winner's curse, multiple comparisons, non-representativeness — small n stays a limit even with strong design.
- **Grade per endpoint**, not per study: a downplayed harm inside a sponsor-funded RCT = "false-reassurance factory" → treat as understated. Stress-tested on 16 cases.

### Full-text resolver
`tools/research_adapters/fulltext_resolver.py` — DOI (or `--title`) → legal free full-text via Unpaywall + Europe PMC, so SCOUTs read whole papers, not just abstracts. Stdlib-only, no API key.
...
3. **Run:**
   ...
   /research organic IG + Telegram growth for AI-education brands consensus high 4h --with-brand ai_mindset  # → marketing domain (auto)
   ```
...
│   ├── creative.md      ← METHODOLOGIST + ART_REVIEWER + creative template
│   ├── marketing.md     ← METHODOLOGIST + CMO_REVIEWER + Framework Library + Quadrant Model + marketing template
│   └── marketing_sources.yaml ← curated per-quadrant source registry (SCOUT-B routes into it)
│
└── examples/
