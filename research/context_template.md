...
| **marketing** | brand profile (ICP/voice/offer), channels | A: principles/academic (L1), B: community buzz (SCOUT-B via /last30days), C: case corpus + failure hunt, D: channel economics, E: competitor recon (ad libraries) |

---

## Marketing domain — Brand Profiles (for `--with-brand`)

<!-- The marketing adapter grounds a personalized run to a specific brand via a Context Profile
     (the same 12-section passport the Content Machine uses). Register brand slug -> profile path here.
     Trigger: `/research <topic> --with-brand <slug>`  OR auto-detect if the topic names a registered brand.
     If the profile's channel handles / "what worked" are 🔴 TODO, the run proceeds but brand-grounding
     degrades to LIMITATIONS — the adapter will NOT invent data. -->

```yaml
brand_profiles:
  # <slug>: <path/to/contexts/<slug>/profile.md>
  # example_brand: path/to/context_profile.md
```
