...
**MANDATORY section in BOTH output types (added 2026-07-02):** `## Frontier Radar` — populated after the FRONTIER_ANALYST runs (step 4d-quater). Every synthesis and every consensus_reference carries it: maturity tiers T0-T4 + the promising-but-unproven shortlist with resolving events. Do not skip even if the topic seems "fully established" — a T0-only radar is a valid, informative result (it says "no live frontier here").

**Context limit:** if >300KB → split into SYNTH-A and SYNTH-B (see `research/agents.md`).

**⚠️ Narrative longread rule (added 2026-07-18, generalized 2026-07-19):** the trigger is **request type, not domain**. «Понять поле / карта темы / изучить X» → user-facing `synthesis_[lang].md` follows `templates/synthesis_longread.md` in ANY domain (company, science, macro, creative, marketing, mixed): narrative, stories-first, NO invented terminology, personal transfer only if explicitly requested, written by the ORCHESTRATOR in one pass — not assembled from subagent chunks. Decision-research («принимать ли X», go/no-go) in any domain + ALL health research keep the 12-section v3.10 layout. Origin: 2026_07_emergence_design user feedback («сухо, непонятно, выдуманная терминология»).
...
- **marketing:** CMO_REVIEWER → `_cmo_review.md`
...
## 4d-quater. FRONTIER_ANALYST (MANDATORY ALWAYS, added 2026-07-02)

> Runs AFTER SYNTHESIZER + DOMAIN_REVIEWER, in PARALLEL with DEVIL'S ADVOCATE, BEFORE FACT-CHECKER.
> Origin: user request to stop the pipeline from silently discarding promising-but-unproven directions ("исследования грейда А обычно не остриё, а догоняющий хвост"). Grade-A evidence lags 5-15y; this layer recovers the low-evidence × high-promise quadrant without hype.

Prompt from `research/prompts.md` section "## FRONTIER_ANALYST".
Reads synthesis/consensus_reference + all streams + deep dives + _methods_review.md (GRADE grades). May use search tools to check the registered-trial pipeline.

Output: `_frontier_radar.md` — maturity-tier table (T0-T4), promise rationale, **resolving event for every T1-T3 item**, ranked promising-but-unproven shortlist, blunt hype shortlist, leading indicators, precedent anchors.

**After FRONTIER_ANALYST — ORCHESTRATOR MUST insert a `## Frontier Radar` section** into synthesis.md / consensus_reference.md (and the bilingual `_ru` copy). Contents: the tier table + the promising-but-unproven shortlist (each with WHY + resolving event + timeline) + the hype shortlist, in plain language. This is a MANDATORY section in every synthesis/consensus output, alongside the existing outcome/finding sections.

**Guardrail:** the section separates the two axes — never inflate the GRADE/confidence (Axis 1) to make something look more promising (Axis 2). A T2 item stays low-evidence; its value is the tracked resolving event, not a borrowed confidence.
