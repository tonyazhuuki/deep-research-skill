...
### A. Sample Size & Design — assess on TWO axes, not one verdict
Design first (always), but N still bounds the *type* of claim. Do not collapse into one "good/bad study" grade — and do not over-correct into "design > N." Both are wrong.

- Was an a priori power analysis conducted? Is n sufficient for a clinically meaningful effect?
- **Axis 1 — internal validity ("is the effect real / what's the mechanism"):** TWO co-equal routes to High (not one).
  - **Route (a) experimental:** randomized **crossover with washout** (within-subject control kills inter-individual variance; n=18 crossover ≈ power of n=60–80 parallel), **dose-dependence** (internal replication), hard outcomes. A small high-resolution RCT ≠ an underpowered weak study.
  - **Route (b) Bradford-Hill in OBSERVATIONAL data (when an RCT is impossible/unethical):** reaches Moderate-High when ALL hold: **very large RR (≥5–10×, beyond ordinary confounding)** + dose-response + consistency in **direction AND magnitude** across independent designs/populations (NOT vote-counting significant positives = that's publication bias) + coherent mechanism + established temporality. **When route (b) holds, the "no randomization" entry in WEAKENS is SUSPENDED for Axis 1** — absence of RCT becomes a precision caveat (Axis 2), not a validity defeater. Example: smoking→lung cancer (RR 10-20×, dose-response, multi-country consistency, mechanism) = Axis 1 **HIGH without an RCT.** Do NOT tank true causation just because there's no RCT.
  - **⚠️ Threshold discriminator (critical):** a large RR upgrades ONLY at RR ≥5–10× + dose-response + multi-design consistency. A **small RR (1.4–1.7×) in a single cohort does NOT upgrade** — healthy-user bias/confounding explains it (this is why smoking = HIGH but an HRT-cohort "protects the heart" = LOW: 10-20× vs ~1.5×).
  - **Mechanism licenses a HYPOTHESIS, not an effect** — never overrides a missing target-population RCT (beta-carotene's antioxidant mechanism sounded great → RCT showed MORE cancer).
  - **Grade per ENDPOINT, not per study:** randomization counts ONLY for the pre-registered primary endpoint at its powered n. Every secondary/exploratory/adverse outcome inherits **observational-grade** validity even inside an RCT. Grade benefit and harm on separate Axis-1 lines. (Vioxx: GI benefit HIGH, but downplayed non-primary CV harm under sponsor COI → a "false-reassurance factory" (mirror of winner's curse) — treat de-emphasized harms as plausibly UNDER-stated; COI is endpoint-asymmetric, raise scrutiny on under-reported harms.)
  - **Within-subject / self-experiment upgrade requires a HARD outcome** — with a soft/self-reported outcome the upgrade is forfeit (allegiance/expectancy bias).
- **Axis 2 — effect-size precision + external validity ("how big / how often / does it apply to the user"):** here small n is STILL a real limit and design does NOT fix it:
  - **Winner's curse** — to hit significance at small n the observed effect must be large → systematically **inflated**. Strong design does not cure this. Small samples + significant p → still flag for effect-size inflation.
  - **Multiple comparisons / multi-omics** — thousands of measurements on ~18 people without FDR-correction & pre-specification = false-discovery factory. Deep data strengthens *mechanism*, but is NOT a power bonus or a free pass.
  - **Non-representativeness** — ~18 people don't cover population heterogeneity; "true in them" ≠ "true in the user/everyone."
  - **Reaction frequency uncovered** — "a subgroup's ALT rose" at n=18 = 3–4 people = hypothesis signal, not a prevalence estimate.
- **Output convention:** grade the two axes separately, e.g. `Snyder Lab fiber RCT, n=18 crossover+multi-omics → effect REAL (Moderate-High) | magnitude/transferability NOT generalizable (Low) — winner's curse + N`. Which axis to cite depends on the claim: "does it exist / mechanism" → Axis 1; "should the user do it / how strong / in whom" → Axis 2 (needs replication at larger N). See HealthOS `_agent/40_evidence.md → Design First, but N Bounds the Claim`.
- Large n does NOT cure bad design either (10M observational with residual confounding ≠ causal).
