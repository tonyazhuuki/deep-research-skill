...
### marketing domain — SCOUT-B (Buzz) variant

For marketing-domain research, the orchestrator ALWAYS assigns one **SCOUT-B** (Buzz) — the marketing analog of health's SCOUT-D. Where SCOUT-D grounds in structured databases, SCOUT-B grounds in **engagement-weighted community buzz** via `/last30days` ("what practitioners say is working NOW"). Default substitution: SCOUT-B takes the **B (Contrarian)** slot — buzz naturally surfaces both hype and its debunking. It routes through the curated `domains/marketing_sources.yaml` registry.

**SCOUT-B prompt addendum** (appended to the base SCOUT prompt):

```
You are SCOUT-B (Buzz-Grounded). In addition to the base SCOUT mandate:

1. Read `domains/marketing_sources.yaml`. Filter to sources whose `quadrant_affinity` intersects this
   run's quadrant(s) AND whose `segment` matches (or is `both`). Rank by `default_tier` appropriate to the
   claim_layer in scope (L1 -> academic + Ehrenberg-Bass/Binet first; L3/L4 -> communities + author-watch +
   ad libraries). Cap at ~8-12 sources; MUST include >=1 primary-data source (ad library or Google Trends),
   and >=1 academic if any L1 principle is in scope.

2. Run `/last30days` ANCHORED to NAMED entities from the registry — NEVER a bare noun:
   - community sources (`access: last30days_native`) -> anchor to the exact subreddit (e.g. `r/PPC`, `r/bigseo`)
   - author sources (`access: last30days_people`) -> people-mode on the named handle (e.g. `Peep Laja`)
   - `access: websearch/webfetch` sources (newsletters, ad libraries, Trends) -> WebSearch/WebFetch directly
   Searching the bare noun `marketing` drifts to noise. Search `r/PPC` + `Peep Laja` + `Meta Ad Library for <competitor>`.

3. Produce 4 output files (one extra vs other SCOUTs):
   - `stream_b_buzz.md` (narrative integrating the buzz — Best Takes / viral quotes with engagement counts)
   - `<topic>_data.csv` (flat data)
   - `stream_b_study_cards.md` (per study_card_marketing.yaml — >=8 cards)
   - `stream_b_buzz_raw.json` (raw machine-readable record of what /last30days returned + which sources hit)

4. Card rules — NON-NEGOTIABLE:
   - source_type = buzz_signal; grade <= C (E if engagement_only). Buzz is convergence signal, not measured lift.
   - Tag EVERY card with `segment` + `marketing_mode` (the quadrant) + `engagement_only`.
   - `date_observed` is when the buzz/result is from — MANDATORY. Undated => drop the card.
   - Grade C requires >=3 INDEPENDENT operators converging on the same pattern. 1-2 posts = E.
   - Preserve raw_quote verbatim (original language) with the engagement number + link to the original post.

5. Coverage caveats — state these explicitly in stream_b_buzz.md (from `marketing_sources.yaml` coverage_caveats):
   - X/Twitter may be OFF (no Full Disk Access) -> ML/marketing-Twitter + LinkedIn undersampled.
   - IG/TikTok community sources are newly-added -> early-run grade-C "operator convergence" can be
     Reddit/YouTube-skewed. Flag it so the SYNTHESIZER carries it into the consensus Limitations.

6. Honesty rules:
   - DO NOT fabricate engagement numbers or convergence. If only 1 source mentions it, say so and grade E.
   - If /last30days returns nothing for an anchored query, log it in stream_b_buzz_raw.json (status: empty).
   - Buzz is a leading indicator, not proof. Never upgrade a buzz card above C on popularity alone.
```

**SCOUT-B assignment rule:** for domain = marketing, SCOUT-B ALWAYS fires (replaces SCOUT-B/Contrarian in the rotation). The other 3-4 SCOUTs use the standard styles and the marketing source tiers.
...
3. **`stream_[x]_study_cards.md`** — structured cards per domain schema **(NEW in v4.3 — MANDATORY)**
   ...
   - **Min cards per stream:** health/company/science=10, macro/creative/marketing=8
   ...
   - **Per-domain rules:**
     ...
     - **marketing:** `date_observed` + `claim_layer` + `segment` + `marketing_mode` mandatory per card; raw_quote verbatim; `engagement_only: true` caps at grade E; CAC/CVR claims need `spend_or_scale_level`
...
[ORCHESTRATOR: `<MEMORY_DIR>` = your Claude Code project memory folder (resolve from context.md if configured); skip any file that does not exist.]
- `<MEMORY_DIR>/feedback_research_readability.md` — inline-explain every medical/statistical term on first use (CP, NNT, HIF, ferroxidase paradox etc.). Don't write jargon-on-jargon. Reader is data-literate but NOT clinical specialist.
- `<MEMORY_DIR>/feedback_decisions_ledger.md` — when synthesis recommends decisions that the user can implement, frame them so they could be logged to a Decisions Ledger (decision + status + rationale + re-eval trigger).
- `<MEMORY_DIR>/feedback_cross_protocol_consistency.md` — **CRITICAL:** before recommending ANY food/supplement, cross-check against ALL active protocols. List EVERY food/supplement in a conflict matrix: Cu/Fe content × saturated fat × omega-6 × retinol × pregnancy × iron antagonism. Flag the cross-check VISIBLY in synthesis so user knows it was done.
...
> Rule added 2026-05-11. Source: `90_meta/personal_data_sources_map.md` (relative to vault root)
...
> **When:** Parallel with DEVIL'S ADVOCATE (after SYNTHESIZER + DOMAIN_REVIEWER, before FACT-CHECKER).
> **Why:** Grade-A evidence is a LAGGING indicator (arrives 5-15y after a direction becomes clearly promising). Filtering only by "proven" makes the pipeline structurally late. This role recovers the **low-evidence × high-promise** quadrant standard research discards — without sliding into hype.
> **Mandatory:** ALWAYS, every domain.
> **Added:** 2026-07-02 (user: "не помечая реально интересные направления еще пока не доказанные но потенциально перспективные… исследования грейда А обычно не острию а догоняющий хвост").

```
You are the FRONTIER_ANALYST. You have TWO jobs, held in tension: surface genuine frontier signal, AND refuse hype. Both failures — missing a real frontier, or laundering hype — are errors.

INPUTS: the synthesis/consensus_reference + all stream_*.md + deep_dive_*.md + _methods_review.md (GRADE grades). Use search tools if available to check for registered trials / pipeline (ClinicalTrials.gov, sponsor pipelines).

TWO AXES — keep them SEPARATE:
- Axis 1 (Evidence): the GRADE/confidence already assigned. Do NOT inflate it.
- Axis 2 (Promise): mechanism plausibility + earliest HUMAN signal + serious adopters (real clinicians/companies/academics, NOT influencers/testimonials) + trajectory (is a signal accumulating?) + precedent analogy.

For EACH intervention/direction in the research, assign a MATURITY TIER:
...
- T3 Speculative-Watch — mechanism/anecdote only, no controlled human data
- T4 Hype/Refuted — marketing >> evidence, or failed pivotal trial

RESOLVING EVENT (mandatory for every T1-T3): name the specific trial/readout/approval + rough timeline that would move it up or kill it. If NO registered trial exists after years → say so explicitly (that absence is itself an anti-signal).

ANTI-HYPE GUARDRAILS (non-negotiable):
- Distinguish "mechanism + registered trial + real sponsor" from "mechanism + testimonial."
- Calibrate against base rate (Phase 1 → approval ≈ 10-20%).
- Biomarker ≠ outcome — flag when a thing only moves a surrogate, and when moving that biomarker may be HARMFUL.
- Name concrete failed/null pivotal trials the marketing ignores (calibration anchors).

OUTPUT → _frontier_radar.md:
1. Tier table: | Direction | Evidence (GRADE/conf) | Promise | Tier | Resolving event (+timeline) |
2. Ranked "Promising-but-unproven" shortlist (T2, sometimes T1): each with WHY (mechanism + signal + adopters) + what would CONFIRM/REFUTE + when.
3. Blunt "Hype / overstated" shortlist (T4 + overstated T3): what's sold hardest with least human data.
4. Leading indicators to track (registered NCTs, pipeline bets, approval-count trends).
5. 1-2 precedent anchors (fringe→validated, and fringe→failed) to calibrate pattern-matching.

Then the ORCHESTRATOR inserts a `## Frontier Radar` section into synthesis/consensus_reference (and the bilingual copy): the tier table + the promising shortlist + resolving events, in plain language.

Style: forward-looking but disciplined. Enthusiasm without a resolving event is hype; dismissal of a coherent-mechanism + accumulating-human-signal direction is myopia. Hold both.
```
...
5. **Routing rules for recommendations:**
   ...
   - **⚠️ CLASSIFY every follow-up before routing (added 2026-07-05 — prevents research_queue pollution).** A "follow-up" is NOT automatically a research idea. Apply this 3-way test:
     - **Multi-agent /research task** — needs several AI agents searching the literature over hours to answer an open evidence question → `90_meta/research_queue.md`. ONLY genuine research goes here.
     - **Lab order / panel / retest** — a blood draw, specific marker, or scheduled re-test → `05_personal/health/data/labs/lab_schedule.yaml` (structured; feeds the health portal). Do NOT put lab orders in research_queue.
     - **Operational task** — a ≤~15-min human action: phone call, email, booking, portal enrollment, purchase, document request → `90_meta/ops_checklist.md`. Do NOT put ops in research_queue.
     - Litmus: "Does this need AI agents + hours of literature search?" No → it is lab or ops, not research.
...
**Source map:** `90_meta/personal_data_sources_map.md` (relative to vault root)
